...
use super::transaction::CertifiedTransaction;
...
pub enum UserTransaction {
    //Use for user transaction without signature
    RawTransaction(RawTransaction),
    //Use for user transaction with signature
    CertifiedTransaction(CertifiedTransaction),
}
impl UserTransaction {
    pub fn digest(&self) -> &TransactionDigest {
        match self {
            UserTransaction::RawTransaction(raw) => raw.digest(),
            UserTransaction::CertifiedTransaction(certificate) => certificate.digest(),
        }
    }
    // pub fn transaction_data(&self) -> &TransactionData {
    //     match self {
    //         UserTransaction::RawTransaction(raw) => raw.transaction_data(),
    //         UserTransaction::CertifiedTransaction(certificate) => certificate.transaction_data(),
    //     }
    // }
    // pub fn epoch(&self) -> EpochId {
    //     match self {
    //         UserTransaction::RawTransaction(raw) => raw.epoch(),
    //         UserTransaction::CertifiedTransaction(certificate) => certificate.epoch(),
    //     }
    // }
}
#[derive(Serialize, Deserialize, Clone, Debug)]
...
impl ConsensusTransaction {
    pub fn new_certificate_message(
        ...
        Self {
            tracking_id,
            kind: ConsensusTransactionKind::UserTransaction(Box::new(
                UserTransaction::CertifiedTransaction(certificate),
            )),
        }
    }
