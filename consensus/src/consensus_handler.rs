...
use crate::{
    authority::{
        AuthorityMetrics, AuthorityPerEpochStore, AuthorityState, ConsensusStats,
        ConsensusStatsAPI, ExecutionIndicesWithStats,
    },
...
impl ConsensusHandlerInitializer {
    ...
    pub fn new_for_testing(
        ...
        Self {
            state: state.clone(),
            consensus_listener: Arc::new(ConsensusListener::default()),
            checkpoint_service,
...
pub struct ConsensusListener {
    senders: Arc<RwLock<Vec<UnboundedSender<ConsensusStreamItem>>>>,
}
...
impl ConsensusListener {
    ...
    pub async fn add_listener(&self, sender: UnboundedSender<ConsensusStreamItem>) {
...
pub struct ConsensusHandler<C> {
    ...
    tx_consensus_output: SenderConsensusOutput,
...
impl<C> ConsensusHandler<C> {
    pub fn new(
        ...
        let (tx_consensus_output, rx_consensus_output) = tokio::sync::mpsc::channel(16);
        spawn_monitored_task!(Self::run(rx_consensus_output, consensus_listener));
...
pub(crate) fn classify(transaction: &ConsensusTransaction) -> &'static str {
    match &transaction.kind {
        ConsensusTransactionKind::UserTransaction(user_tx) => {
            // if certificate.contains_shared_object() {
            //     "shared_certificate"
            // } else {
            //     "owned_certificate"
            // }
            match user_tx.as_ref() {
                UserTransaction::RawTransaction(_) => "uncertificate",
                UserTransaction::CertifiedTransaction(_) => "certificate",
            }
        }
...
impl SequencedConsensusTransaction {
    ...
    pub fn is_user_tx_with_randomness(&self, randomness_state_enabled: bool) -> bool {
        ...
        //20240607 TaiVV: Read more about randomness
        return false;
    }
