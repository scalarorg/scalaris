...
use crate::transaction::{
    CertifiedTransaction, SenderSignedData, TransactionKey, VerifiedSignedTransaction,
};
...
use sui_core::checkpoints::EpochStats;
...
use crate::message_envelope::{
    TrustedEnvelope, TrustedExecutableTransaction, VerifiedExecutableTransaction,
};
...
use crate::messages_consensus::check_total_jwk_size;
...
impl AuthorityPerEpochStore {
    ...
    //         .collect::<Result<Vec<_>, _>>()?)
    // }
    // 20240606: TaiVV in Execution layer
    // fn should_defer(
    //     &self,
    //     cert: &VerifiedExecutableTransaction,
    //     commit_round: Round,
    ...
    >(
        ...
        //         })
        //         .collect();
        let deferred_txs: Vec<(DeferralKey, Vec<VerifiedSequencedConsensusTransaction>)> = vec![];
        let previously_deferred_tx_digests = HashMap::default();
        ...
        let (transactions_to_schedule, system_transactions, notifications, lock, final_round) =
    ...
    pub(crate) async fn process_consensus_transactions(
        ...
        Vec<UserTransaction>, // transactions to schedule
        ...
        let mut user_transactions = VecDeque::with_capacity(transactions.len() + 1);
        ...
        for tx in transactions {
            ...
            // };
            let execution_cost = &mut shared_object_congestion_tracker;
            match self
                ...
                .await?
            {
                ConsensusCertificateResult::UserTransaction(cert) => {
                    notifications.push(key.clone());
                    user_transactions.push_back(cert);
                }
        ...
        let user_transactions: Vec<_> = user_transactions.into();
        //TODO: 20240606 TaiVV - Review how to implement this
    ...
    async fn process_consensus_transaction(
        ...
        match &transaction {
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
                kind: ConsensusTransactionKind::UserTransaction(user_tx),
                ..
            }) => {
                ...
                // }
                // Collect then Send to the external layer
                Ok(ConsensusCertificateResult::UserTransaction(
                    *user_tx.clone(),
                ))
            }
            SequencedConsensusTransactionKind::External(ConsensusTransaction {
    ...
    fn process_consensus_system_transaction(
        ...
        ConsensusCertificateResult::SystemTransaction(system_transaction.clone())
    }
